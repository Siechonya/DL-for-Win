...
class CnnTransformerAutoencoder(nn.Module):
    """
    CNN + Transformer Encoder + Bi-LSTM Decoder
    Self-attention 同时比较序列任意位置→天然区分激波(永久阶跃)和磁洞/孤子(局域回弹)
    物理特征 + 原型对比损失保持不变, 防止模型走偏
    """
    def __init__(self, input_size=4, cnn_channels=16, d_model=20, nhead=4,
                 num_encoder_layers=3, dim_feedforward=80, latent_dim=64,
                 decoder_hidden=128, decoder_layers=2, dropout=0.1):
        super().__init__()
        # 1. 1D-CNN 前置提取器
        self.cnn = nn.Sequential(
            nn.Conv1d(in_channels=input_size, out_channels=cnn_channels, kernel_size=5, padding=2),
            nn.LeakyReLU(0.1)
        )
        # d_model = input_size + cnn_channels = 4 + 16 = 20

        # 2. 可学习位置编码
        self.pos_encoding = nn.Parameter(torch.randn(1, 300, d_model) * 0.02)

        # 3. Transformer 编码器 (d_model 小 → 注意力矩阵 [300,300] 可控 → 显存安全)
        ...
        # 4. 混合池化 + 降维 → latent
        self.fc_reduce = nn.Linear(d_model * 2, latent_dim)  # 40 → 64

        # 5. Bi-LSTM 解码器 (保持不变)
        self.decoder_lstm = nn.LSTM(latent_dim, decoder_hidden, decoder_layers,
                                     batch_first=True, bidirectional=True)
        self.decoder_fc = nn.Linear(decoder_hidden * 2, input_size)

    def encode(self, x):
        # x: [batch, 300, 4]
        x_cnn_in = x.permute(0, 2, 1)
        cnn_out = self.cnn(x_cnn_in).permute(0, 2, 1)  # [batch, 300, 16]
        x_combined = torch.cat([x, cnn_out], dim=2)      # [batch, 300, 20]

        # 位置编码 + Transformer
        x_combined = x_combined + self.pos_encoding[:, :x_combined.size(1), :]
        out = self.transformer_encoder(x_combined)  # [batch, 300, 20]

        # 混合池化
        pooled_max, _ = torch.max(out, dim=1)
        pooled_avg = torch.mean(out, dim=1)
        pooled_concat = torch.cat([pooled_max, pooled_avg], dim=1)  # [batch, 40]
    ...
    def forward(self, x, return_embedding=False):
        ...
        z_rep = latent_z.unsqueeze(1).repeat(1, seq_len, 1)
        dec_out, _ = self.decoder_lstm(z_rep)
        reconstructed = self.decoder_fc(dec_out)
        if return_embedding:
...
def train_autoencoder(model, train_dataloader, val_dataloader, proto_dataloader, device,
    ...
    scheduler = torch.optim.lr_scheduler.ReduceLROnPlateau(optimizer, 'min', factor=0.5, patience=4, min_lr=1e-4)
...
model = CnnTransformerAutoencoder(input_size=4, cnn_channels=16, d_model=20, nhead=4,
                                  num_encoder_layers=3, dim_feedforward=80,
                                  latent_dim=64, decoder_hidden=128, decoder_layers=2).to(device)
