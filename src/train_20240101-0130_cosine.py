...
def physical_contrastive_loss(embeddings, labels, phys_features, margin=2.0):
    ...
    # 余弦距离：L2 归一化后欧氏距离等价于 sqrt(2 - 2*cos_sim)
    embeddings_norm = F.normalize(embeddings, p=2, dim=1)
    dist_matrix = torch.cdist(embeddings_norm, embeddings_norm, p=2)
...
if __name__ == "__main__":
    workspace = '..\\'
    trainset_path = os.path.join(workspace, 'trainset_20240101-0130')
...
def test_clustering(model, test_data_preprocessed, prototypes_preprocessed_dict, device, n_std=2.0, batch_size=256):
    ...
    # 余弦距离
    dist_matrix = cdist(test_embs, centers_matrix, metric='cosine') 
    ...
    # --- 6. 模块三：范本中心两两之间的距离 ---
    print("\n" + "-" * 35)
    print("Prototype Centers Distance Matrix (Cosine):")
    # pdist 计算两两距离，squareform 将其转为方阵
    dist_matrix_centers = squareform(pdist(centers_matrix, metric='cosine'))
